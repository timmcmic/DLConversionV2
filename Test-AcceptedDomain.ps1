    ...
    Function Test-AcceptedDomain
     {
        ...
        else 
        {
            ...
            try {
                $functionOnMicrosoftDomain = get-acceptedDomain | where {$_.domainName -like "*$functionNameTest"} -errorAction Stop
            }
            catch {
                out-logfile -string $_
                out-logfile -string "Unable to capture accepted domains for interpretation -> error." -isError:$TRUE
        ...
        out-logfile -string $functionOnMicrosoftDomain.domainName
