    ...
    do 
    {
        ...
        if ($remainingAddresses -ge $maxThreadCount)
        {
            ...
            for ($forCounter = 0 ; $forCounter -lt $maxThreadCount ; $forCounter ++)
            {
                ...
                #Start-Job -InitializationScript {Import-Module c:\test\DLConversionV2} -ScriptBlock {Start-DistributionListMigration -groupSMTPAddress $groupSMTPAddresses[$ArrayLocation+$forCounter] -globalCatalogServer $globalCatalogServer -activeDirectoryCredential $activeDirectoryCredential -logFolderPath $logFolderPath -aadConnectServer $aadConnectServer -aadConnectCredential $aadConnectCredential -exchangeServer $exchangeServer -exchangeCredential $exchangecredential -exchangeOnlineCredential $exchangeOnlineCredential -exchangeOnlineCertificateThumbPrint $exchangeOnlineCertificateThumbPrint -exchangeOnlineOrganizationName $exchangeOnlineOrganizationName -exchangeOnlineEnvironmentName $exchangeOnlineEnvironmentName -exchangeOnlineAppID $exchangeOnlineAppID -exchangeAuthenticationMethod $exchangeAuthenticationMethod -retainOffice365Settings $retainOffice365Settings -dnNoSyncOU $dnNoSyncOU -retainOriginalGroup $retainOriginalGroup -enableHybridMailflow $enableHybridMailflow -groupTypeOverride $groupTypeOverride -triggerUpgradeToOffice365Group $triggerUpgradeToOffice365Group -retainFullMailboxAccessOnPrem $retainFullMailboxAccessOnPrem -retainSendAsOnPrem $retainSendAsOnPrem -retainMailboxFolderPermsOnPrem $retainMailboxFolderPermsOnPrem -retainFullMailboxAccessOffice365 $retainFullMailboxAccessOffice365 -retainSendAsOffice365 $retainSendAsOffice365 -retainMailboxFolderPermsOffice365 $retainMailboxFolderPermsOffice365 -useCollectedFullMailboxAccessOnPrem $useCollectedFolderPermissionsOnPrem -useCollectedFullMailboxAccessOffice365 $useCollectedFullMailboxAccessOffice365 -useCollectedSendAsOnPrem $useCollectedSendAsOnPrem -useCollectedFolderPermissionsOnPrem $useCollectedFolderPermissionsOnPrem -useCollectedFolderPermissionsOffice365 $useCollectedFolderPermissionsOffice365 -threadNumberAssigned $forThread -totalThreadCount $loopThreadCount}
                Start-Job -InitializationScript {Import-Module c:\test\DLConversionV2} -ScriptBlock {Start-DistributionListMigration -groupSMTPAddress $groupSMTPAddresses[$ArrayLocation+$forCounter]}
