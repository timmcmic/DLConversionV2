    ...
    do 
    {
        ...
        if ($remainingAddresses -ge $maxThreadCount)
        {
            ...
            for ($forCounter = 0 ; $forCounter -lt $maxThreadCount ; $forCounter ++)
            {
                ...
                Start-Job -Name $jobName -InitializationScript {import-module c:\repository\DLConversionV2\DLConversionV2.psd1 -Force} -ScriptBlock { Test-PreMigration -groupSMTPAddress $args[0] -globalCatalogServer $args[1] -activeDirectoryCredential $args[2] -logFolderPath $args[3] -exchangeOnlineCredential $args[4] -exchangeOnlineCertificateThumbPrint $args[5] -exchangeOnlineOrganizationName $args[6] -exchangeOnlineEnvironmentName $args[7] -exchangeOnlineAppID $args[8] -useCollectedFullMailboxAccessOffice365 $args[9] -azureADCredential $args[10] -azureEnvironmentName $args[11] -azureTenantID $args[12] -azureApplicationID $args[13] -azureCertificateThumbprint $args[14] -allowTelemetryCollection $args[15] -allowDetailedTelemetryCollection $args[16]} -ArgumentList $groupSMTPAddresses[$arrayLocation + $forCounter],$globalCatalogServer,$activeDirectoryCredential,$originalLogFolderPath,$exchangeOnlineCredential,$exchangeOnlineCertificateThumbPrint,$exchangeOnlineOrganizationName,$exchangeOnlineEnvironmentName,$exchangeOnlineAppID,$useCollectedSendAsOnPrem,$azureADCredential,$azureEnvironmentName,$azureTenantID,$azureApplicationID,$azureCertificateThumbprint,$allowTelemetryCollection,$allowDetailedTelemetryCollection
